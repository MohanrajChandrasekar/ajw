...
export class BookingDetailsComponent implements OnInit {
  ...
  refreshData(dashboardDate) {
    ...
    this.dashboardService.getMawb(dashboardDate).subscribe(data => {
      this.temp = data;
      //this.dialogRef.close();
      var keys = ['amount']//data.columns.slice(1);
      data.forEach(element => {
        element.total = element.amount;
      });
      x.domain(data.map(function (d) { return d.MAWB; }));
      ...
      g.append("g")
        ...
        .enter().append("rect")
        .attr("x", function (d) { return x(d.data.MAWB); })
        .attr("y", function (d) { return y(d[1]); })
        .attr("height", function (d) { return y(d[0]) - y(d[1]); })
        .attr("width", x.bandwidth())
        .on("mouseover", function (d) {
          var txt = '<br/>Total Amount:'+ d.data.total;
          div.transition()
            .duration(200)
            .style("opacity", 1);
          div.html(txt)
      ...
      //   .attr("dy", "0.32em")
      //   .text(function (d) {
      //     if (d == 'amount') { return 'Total Amount'; }
        // });
    },
    err => {
      throw err;
